...
const ARCHITECT_DISPATCH_NUDGE = [
  ...
  "  - otherwise: the pipeline failed partway, but `report.files` may",
  "    still hold a materialized best-effort project (the graph was",
  "    built, just didn't pass integration or finalize). Options:",
  "      (a) `FINAL_FILES(report)` to return what was built — usually",
  "          the right move; don't re-design from scratch.",
  "      (b) Only if report.files is empty AND report.phase tells you",
  "          what went wrong: call design_plan() again with a fix.",
  "    Do NOT call design_module / design_function by hand to re-design",
  "    a second version — it clutters the graph with phantom modules.",
  "",
