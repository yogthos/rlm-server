...
export async function buildImplementerPrompt(
  ...
  const lines = [
    ...
    "",
    "   **VITEST TYPE USAGE** — `vi` is a VALUE exported from 'vitest',",
    "   NOT a TypeScript namespace. Writing `let m: vi.Mock;` raises",
    "   `TS2503: Cannot find namespace 'vi'`. To annotate a mock with",
    "   a TYPE, import the type separately from vitest:",
    "     import { describe, it, expect, vi } from \"vitest\";",
    "     import type { Mock, Mocked } from \"vitest\";",
    "     let handler: Mock;                         // ✓ correct",
    "     const api: Mocked<typeof realApi> = vi.mocked(realApi);  // ✓",
    "     // AVOID:",
    "     let handler: vi.Mock;                      // ✗ TS2503",
    "     let handler: vi.Mocked<X>;                 // ✗ TS2503",
    "   Runtime calls (`vi.mock()`, `vi.fn()`, `vi.mocked()`) are fine —",
    "   `vi` IS a value. It's only as a TYPE namespace that it fails.",
    "   Same rule for jest: `import type { Mock } from \"@jest/globals\"`",
    "   and use `Mock`, not `jest.Mock`.",
    "",
    integrationNeeded
